...
const TRANSACTION_HEADER = [
  ...
  "structured_summary",
  "heuristics",
  "raw_category_suggestion",
  "parsed_temporal_phrase",
  "meta_source",
  "meta_medium",
  "meta_target_party",
].join(",");
...
function buildTransactionCsvRow(transaction: NormalizedTransaction): string {
  return serializeCsvRow([
    ...
    transaction.structuredSummary,
    transaction.meta.heuristics.join("|"),
    transaction.meta.rawCategorySuggestion ?? "",
    transaction.meta.parsedTemporalPhrase ?? "",
...
function deserializeNormalizedTransaction(values: string[]): NormalizedTransaction {
  ...
  while (normalizedValues.length < 19) {
  ...
  const heuristics = splitList(heuristicsRaw);
  ...
  const normalized: NormalizedTransaction = {
    ...
    recordedAt,
    eventDate,
    direction: direction as LogCashTransactionPayload["direction"],
    amount: Number.isNaN(amount) ? 0 : amount,
    currency,
    category,
    flavor: flavor as CategorizationResult["flavor"],
    description,
    rawText,
    tags,
    structuredSummary,
    meta: {
      source: metaSourceRaw && metaSourceRaw.length > 0 ? metaSourceRaw : "mill-chat",
      heuristics,
    },
  };
  ...
  if (rawCategorySuggestion) {
    normalized.meta.rawCategorySuggestion = rawCategorySuggestion;
  }

  if (parsedTemporalPhrase) {
    normalized.meta.parsedTemporalPhrase = parsedTemporalPhrase;
  }

  if (metaMediumRaw) {
    normalized.meta.medium = metaMediumRaw;
  }

  if (metaTargetPartyRaw) {
    normalized.meta.targetParty = metaTargetPartyRaw;
  }
