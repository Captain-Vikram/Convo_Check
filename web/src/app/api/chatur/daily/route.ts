...
import { runCoach } from "@/runtime/chatur/coach-agent";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const briefing = await runCoach({ trigger: "daily", ownerId });
    return NextResponse.json({ success: Boolean(briefing), briefing });
  } catch (error) {
