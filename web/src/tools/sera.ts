...
import { addToWishlist, getWishlist, type WishlistItem } from '../runtime/sera/wishlist-manager';
import {
  searchAmazonProduct,
  type AmazonSearchResult,
} from '../runtime/sera/services/search-service';
import {
...
import { logger } from '../runtime/shared/logger';
...
type WishlistInput = Parameters<typeof addToWishlist>[0];
...
export const viewWishlistTool = createTool({
  ...
  execute: async () => {
    const items: WishlistItem[] = await getWishlist();
    if (items.length === 0) {
    ...
    items.forEach((item, i) => {
      lines.push(
        `${i + 1}. ${item.name} (Current: ${item.currentPrice}, Target: ${item.desiredPrice})`,
      );
    });
    return lines.join("\n");
  },
...
export const updateWishlistToolDefinition: ToolDefinition = {
  name: "update_wishlist_item",
  description: "Update an item in the user's wishlist, usually to set a new target price.",
  parameters: [
    {
      name: "reference",
      type: "string",
      description:
        "The reference to the item, e.g., '#1', 'last', or a product name fragment.",
      required: true,
    },
    {
      name: "desiredPrice",
      type: "string",
      description: "The new target price for the item.",
      required: true,
    },
  ],
};

export const removeFromWishlistToolDefinition: ToolDefinition = {
  ...
  parameters: [
    {
      ...
      type: "string",
      description:
        "The reference to the item to remove, e.g., '#1', 'last', or a product name fragment.",
      required: true,
    },
  ],
};

export const clearWishlistToolDefinition: ToolDefinition = {
  name: "clear_wishlist",
...
/**
 * Create the AI SDK tool for getting product details from a URL.
 * This is a no-op here as it's handled by the Sera agent's internal logic.
 */
